...
- **深色 / 浅色主题** — 内置深色和浅色两套主题，支持通过 `setTheme()` 运行时动态切换
- **移动端优先** — 触控优化，可拖拽悬浮按钮带边缘吸附，响应式面板
- **体积小巧** — gzip 后约 23KB（含内置插件）
...
| `nc.setTheme(theme)` | 切换主题（`'dark'` 或 `'light'`） |
...
| `nc.exportForAI()` | 导出包含报错、关联网络请求和脱敏 DOM 快照的 Markdown |
| `nc.copyForAI()` | 一键复制 AI 排障 Markdown 到剪贴板 |
